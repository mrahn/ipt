...
\path{src/third_party/CRoaring/}, used for the Roaring baseline.
...
no measured hot path throws. Cuboid restriction is benchmarked through
each layout's native \texttt{restrict} operation rather than through a
shared select-to-IPT pipeline. The hot cuboid-search and
cuboid-local translation kernels are the same as in the deployed
C++14 implementation integrated downstream into a
...
($\kappa$ for IPTs), four point-query timings, and a restriction sweep.
...
For restriction, each implementation is queried with 32 random
axis-aligned sub-cuboids at each relative window volume
$d \in \{0.001, 0.01, 0.1, 1.0\}$, and the benchmark records
\texttt{restrict\_d*} latency. Auxiliary counters for restricted point
count and restricted bytes are emitted for debugging and extraction,
but are not used in the paper tables.
...
random \texttt{pos}/\texttt{at} samples and the same \texttt{restrict}
...
\textsc{GreedyPlusMerge}) with a native \texttt{restrict} method that
returns the same layout type restricted to the query window. The
point-level baselines rebuild from surviving points. \textsc{LexRun}
and \textsc{RowCSR-k*} clip their stored runs and rows directly. The
bitmap layouts rebuild restricted bitmaps over the intersection grid.
...
to the \textsc{SortedPoints} reference after restriction.

The full 18-platform/compiler sweep emits \texttt{restrict\_d*}
measurements for every algorithm and scenario, so the asymptotic
claims above are checked across the same matrix as \texttt{at} and
\texttt{pos}. These numbers compare the natural restriction workload
for each layout rather than a shared select-to-IPT rebuild. The
comparison is therefore symmetric across layouts. It measures overlap
search plus native restricted-layout construction on exactly the
queried window.
...
\texttt{restrict}, and steady-state mmap-backed throughput using the
...
remains much smaller than the point-list baselines, mirroring the
in-memory footprint, while \textsc{Roaring}, \textsc{DenseBitset}, and
\textsc{BlockBitmap} can be smaller still in absolute bytes but pay
for that compactness on query throughput. Second, the IPT's cold
first-response latencies after \texttt{mmap} show that mapping is not
followed by reconstruction or pointer fix-up. Third, the steady-state
mmap-backed throughput stays in the same band as the in-memory
measurements, so mmap is a viable runtime substrate rather than only a
load path. As in Section~\ref{section:selection}, \texttt{c0r0e0l0}
trades space for a small \texttt{restrict}-latency penalty.
...
\Description{Table summarising bytes on disk, bytes per point, cold
  first-response latency for pos / at / restrict, and steady-state
...
\Description{Table of aggregate measurements for the
  ...
  restrict latency at two density levels, with geometric mean
...
\Description{Table of IPT over SortedPoints time ratios on the
  ...
  pos_random, pos_all, at_random, at_all, and restrict at
...
\texttt{at}/\texttt{pos} gap persists at $d=5$, and
\texttt{restrict} stays in the microsecond range up to a $d=0.1$
relative window, confirming that the cuboid-algebra path still scales
with structure rather than with $N$.
...
\texttt{restrict} path remains orders of magnitude faster than
\texttt{SortedPoints}.
...
\begin{itemize}
  ...
  \item \texttt{make plots} regenerates every figure in this paper
    by compiling the per-figure extractors in
    \texttt{results/plot/*.cpp}, running them over \texttt{results/},
    and invoking gnuplot on the \texttt{results/plot/*.gp} scripts.
  \item \texttt{make paper} re-runs \texttt{pdflatex} and
    \texttt{bibtex} on \texttt{paper/IPT.tex}.
\end{itemize}
