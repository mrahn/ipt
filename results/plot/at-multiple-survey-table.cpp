// Aggregate per-scenario at_random ns/at for the multiple-survey
// scenarios on the reference platform and default cache
// configuration, and emit a stand-alone LaTeX table with one row per
// algorithm (IPT first, absolute ns/at) and one column per scenario
...
auto main() -> int
{
  ...
  std::printf ("%% Auto-generated by results/plot/at-multiple-survey-table.cpp.\n");
  ...
  // IPT rows: absolute ns/at and the equivalent throughput Mat/s.
  auto const& ipt {algorithms.front()};
  std::printf ("%s & ns/\\texttt{at}", ipt.label.c_str());
  std::ranges::for_each
    ( kept_scenarios
    , [&] (auto const& s)
      {
