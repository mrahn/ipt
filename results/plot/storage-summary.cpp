...
namespace
{
  ...
  {
    return
      { { "greedy-plus-merge-mmap"     , "\\textsc{IPT} (\\texttt{c0r1e1l0})" }
      ...
      , { "row-csr-k1-mmap"            , "\\textsc{RowCSR-k1}" }
        , { "row-csr-k2-mmap"            , "\\textsc{RowCSR-k2}" }
      };
  }
...
auto main() -> int
{
  ...
  std::printf ("{\\setlength{\\tabcolsep}{2.5pt}\n");
  std::printf ("\\begin{tabular}{lcrrrrrrrrrcrrrr}\n");
  std::printf ("\\toprule\n");
  auto const print_inner_rule
    { []()
      {
        std::printf
          ( "\\cmidrule(r){1-1}"
