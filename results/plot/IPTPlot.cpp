...
namespace ipt_plot
{
      ...
      , [&] (auto const& entry)
        {
          ...
          if (!std::string_view {id}.starts_with ("os_"))
          {
            return;
          }
    ...
    ) -> std::string
  {
    ...
    if ( auto const* preferred
           { find_preferred ("AMD EPYC 9454", "Clang", "Rocky 10.1")
           }
       )
    {
      return preferred->id;
    ...
    ) -> std::optional<FileMetadata>
  {
    ...
    auto constexpr restrict_structured_independent_prefix
      {std::string_view {"restrict-structured-cache-independent-"}};
    if ( name.starts_with (restrict_structured_independent_prefix)
      && name.ends_with (".txt")
       )
    {
      auto const config
        { name.substr
          ( restrict_structured_independent_prefix.size()
          , name.size() - restrict_structured_independent_prefix.size() - 4
          )
        };
      if (is_cache_config (config))
      {
    ...
    auto constexpr restrict_structured_prefix
      {std::string_view {"restrict-structured-"}};
    if (name.starts_with (restrict_structured_prefix) && name.ends_with (".txt"))
    {
      auto const config
        { name.substr ( restrict_structured_prefix.size()
                      , name.size() - restrict_structured_prefix.size() - 4
                      )
        };
      if (is_cache_config (config))
      {
    ...
    // The 5D restriction file holds both the cache-dependent IPT rows
    // and the cache-independent baseline rows for the 5D stress
    // scenario in a single file. Consumers select rows by scenario and
    // metric rather than by cache kind.
    auto constexpr restrict_5d_prefix {std::string_view {"restrict-5d-"}};
    if (name.starts_with (restrict_5d_prefix) && name.ends_with (".txt"))
    {
      auto const config
        { name.substr ( restrict_5d_prefix.size()
                      , name.size() - restrict_5d_prefix.size() - 4
                      )
        };
      if (is_cache_config (config))
      {
