// Extract construct_ns/point for multiple-survey scenarios on the
// reference platform and default cache configuration. Equivalent to
// results/plot/construct-multiple-survey.pl.
...
auto main() -> int
{
  ...
  visit_rows
    ( [&] (auto const& platform, auto const& meta, auto const& row)
      {
        if (platform.id != platform_id || row.value ("metric") != "construct")
        {
          return;
        }
