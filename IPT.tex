...
achieves the optimal IPT size in $O(dN)$ time on sorted input.
Benchmarks on twenty x86 platform/compiler combinations with regular
grids, random subsets, and structured synthetic multiple-survey layouts show that the IPT
reaches $169{-}180$\,Mpos/s on the tested regular grids while staying
within $1\%$ of pure mixed-radix rank/unrank on that regular case,
uses $13.8\%$ of the memory of a sorted-points table on the structured
multiple-survey layouts, and accelerates \texttt{pos} by
...
of occupied codes. It therefore gives an ordering rather than a
...
(i)~$\operatorname{lub}(R) = e-s$, (ii)~$|R| = (e-b)/s$ is an exact integer operations with no rounding.
...
Adaptive space
  & Metadata is $O(d\kappa)$. On (strided-)regular data $\kappa = 1$
    ...
    On the structured multiple-survey scenarios the IPT uses $13.8\%$
    of the sorted-points footprint with the selected default
    configuration and $10.5\%$ in the minimal-footprint
    configuration. \\
Adaptive access time
  & \texttt{at} is $O(d+\log\kappa)$ and \texttt{pos} is
    $O(d\log\kappa)$.
    On (strided-)regular data ($\kappa = 1$) both reduce to $O(d)$.
    Measured \texttt{pos} reaches ${\sim}180$\,Mpos/s on a regular
    $100{\times}100{\times}100$ grid. \\
Flat layout
...
By contrast, the flat IPT stores two cuboid records with base, stride,
and bound for each ruler, plus two prefix sums, for a total of
$14$~scalars.
...
differ is where the two representations genuinely
differ.
...
\toprule
& \textbf{(Strided-)regular} ($\kappa=1$)
& \textbf{Highly regular} ($\kappa\ll N$)
& \textbf{Highly irregular} ($\kappa\approx N$) \\
\midrule
\multicolumn{4}{@{}l}{\emph{Storage}} \\
\quad Flat IPT        & $\Theta(d)$ & $\Theta(d\kappa)$ & $\Theta(dN)$ \\
\quad Recursive IPT   & $\Theta(d)$ & $O(d\kappa)$      & $\Theta(dN)$ \\[4pt]
\multicolumn{4}{@{}l}{\emph{\texttt{at}}} \\
\quad Flat IPT        & $O(d)$ & $O(d+\log\kappa)$  & $O(d+\log N)$ \\
\quad Recursive IPT   & $O(d)$ & $O(d+\log\kappa)$--$O(d\log\kappa)$ & $O(d+\log N)$--$O(d\log N)$ \\[4pt]
\multicolumn{4}{@{}l}{\emph{\texttt{pos}}} \\
\quad Flat IPT        & $O(d)$ & $O(d\log\kappa)$\rlap{${}^{\dagger}$}  & $O(d\log N)$\rlap{${}^{\dagger}$} \\
\quad Recursive IPT   & $O(d)$ & $O(d\log\kappa)$\rlap{${}^{\ddagger}$} & $O(d\log N)$\rlap{${}^{\ddagger}$} \\[4pt]
\multicolumn{4}{@{}l}{\emph{Selection} ($t$~candidates, $m_k$~nodes visited at level~$k$)} \\
\quad Flat IPT        & $O(d)$ & $O(d\log\kappa+dt)$  & $O(d\log N+dt)$ \\
\quad Recursive IPT   & $O(d)$ & $O(\log\kappa{\cdot}\textstyle\sum m_k)$ & $O(\log N{\cdot}\textstyle\sum m_k)$ \\
\bottomrule
\multicolumn{4}{@{}l}{\footnotesize ${}^{\dagger}$\,tuple comparisons;\;
  ${}^{\ddagger}$\,scalar comparisons}
\end{tabular}
...
bitmaps. \texttt{LexRun} stores maximal lexicographic runs in the fastest
dimension. \texttt{RowCSR-k1} and \texttt{RowCSR-k2} are compressed-sparse-row
layouts with one and two row dimensions, respectively.
Together these baselines span explicit-point, dense-occupancy,
run-length, row-compressed, and regular-grid designs. They do not
exhaust every practical competitor, especially tree-based or
...
\paragraph{Differences from the industrial implementation.}
The industrial implementation shares the same algorithms but uses
pre-allocated or memory-mapped storage (at most one allocation), targets
C++14, and reports errors via an \texttt{expected}-style return type.
The benchmark artifact instead uses throwing query methods, targets
C++23, uses \texttt{std::vector} with doubling growth, and exposes the
selection pipeline needed for the
Section~\ref{section:results-selection} measurements: \texttt{IPT}
provides \texttt{select}, and \texttt{restrict} is evaluated by feeding
that view into the \texttt{std::from\_range} constructor. These
differences change the surrounding API and storage strategy, but not the
ordered-entry search and cuboid-local translation kernels that dominate
the measured hot paths. No exception is thrown on any measured hot path.
...
The two dense occupancy baselines (DenseBitset and BlockBitmap) are
restricted to the seven smaller structured scenarios because their cost
scales with the full bounding-grid volume rather than with the number of
occupied points.
...
The implementation exposes four boolean compile-time flags (\texttt{c},
\texttt{r}, \texttt{e}, \texttt{l}) that each control whether
a derived quantity (such as a ruler size, a cuboid size, or an entry's end
index or bounding point) is cached as an additional stored field, yielding
...
frontier point, but its metadata grows to $1.70\times$ of
\texttt{c0r0e0l0}.
...
The IPT therefore recovers its build cost after approximately $N/7$
\texttt{pos} queries, where $N$ is the number of input points.
Any workload that issues at least one query per seven stored points (like repeated-timestep simulations with fixed geometry do)
amortises construction within the first pass.
...
The expected cost of one full enumeration is
\[
  O(\log\kappa + d\,t + d\,r),
\]
...
returned. The $\log\kappa$ term is the one-shot binary search; $d\,t$ is
the per-candidate intersection cost; $d\,r$ is the per-result yield
cost.
...
irregular data retains substantial cuboid structure. In that regime the
...
on those layouts, and also preserves the regular-grid case closely with
the \texttt{c0r1e1l0} cache configuration.
