...
adaptive flat exact representation of irregular $d$-dimensional point
sets. It stores the domain as a strictly ascending sequence of strided
cuboids with prefix sums. Queries locate the cuboid by binary search
and then apply the local Horner mapping. Storage scales with the data
irregularity~$\kappa$. Index-to-point (\texttt{at}) costs $O(d +
\log\kappa)$ and point-to-index (\texttt{pos}) costs $O(d\log\kappa)$,
both collapsing to~$O(d)$ for (strided-)regular data where
$\kappa=1$. The IPT is closed under geometric restriction, so each
processing unit can store only the metadata for its sub-region. We
prove that greedy-plus-combine constructs an optimal IPT in linear
time on sorted input.

Empirically, benchmarks on eighteen x86 platform/compiler combinations
show that the IPT matches the pure mixed-radix rank/unrank performance
in regular grids within $3\,\%$. For structured application layouts
the IPT uses about $8.8\%$ of the memory of a \texttt{SortedPoints}
table and improves \texttt{pos} throughput by about $2.25\times$.
\end{abstract}
...
middle: one global binary search resolves irregularity, and the inner
computation remains Horner. Its advantage is a flat,
data-independent layout.
...
\begin{itemize}[leftmargin=*,nosep]
  \item an adaptive flat representation for irregular $d$-dimensional
    point sets that preserves (strided-)regular-grid rank/unrank,
  \item a design-space analysis that links the IPT to regular-grid
    indexing and makes the flat-versus-recursive trade-off explicit,
  \item practical linear-time construction algorithms
    (greedy-plus-merge and greedy-plus-combine), with
    greedy-plus-combine proved optimal,
  \item complexity analyses for rank/unrank, storage, geometric
    operations, and geometric decomposability, and
  \item a benchmark implementation and empirical evaluation on eighteen
...
To allow for incremental introduction into existing code bases that
work with regular data and to be efficient means to implement all of
the following requirements in a single data structure:
...
Since the minimum-$\kappa$ decomposition is not always unique (see the
non-uniqueness example in~\S\ref{section:decomposition-problem}), it is
natural to ask which optimum the greedy-plus-combine selects.

\begin{proposition}[Canonical decomposition]
\label{proposition:canonical-decomposition}
Let $(C_0, \ldots, C_{\kappa-1})$ be the decomposition produced by
greedy-plus-combine on~$D$. Then it is the unique minimum-$\kappa$
decomposition whose size sequence
$(|C_0|, |C_1|, \ldots, |C_{\kappa-1}|)$ is lexicographically
greatest among all minimum-$\kappa$ decompositions of~$D$.
We call it the \emph{canonical} optimal decomposition of~$D$.
\end{proposition}

The same induction used in Appendix~\ref{proof:gpc-optimality} proves
this characterization. It maximises $|C_0|$ first, then maximises
$|C_1|$ among all optima with that value of $|C_0|$, and so on.
...
\midrule
Time (sorted input) & $O(dN)$ & $O(dN)$ & naive $\Theta(dN^3)$ \\
...
times the optimum, and no upper bound is currently known. Determining
whether greedy-plus-merge admits any finite approximation guarantee as
a function of~$d$ is an open problem.
...
The dynamic programming algorithm also guarantees the minimum~$\kappa$, but its naive cost is $\Theta(dN^3)$. Even a sharper $\Theta(dN^2)$ implementation would remain impractical for large inputs.
...
\medskip
\noindent
The summary is therefore as follows. In the (strided-)regular case
...
library is the vendored CRoaring copy in \path{third_party/CRoaring/},
used for the Roaring baseline.
...
only the per-fragment translation work reused by \texttt{at} and
\texttt{pos}, but also the candidate-enumeration cost of the selection
loop and the rebuild-from-range step used for restriction.
...
\Description{A table listing the eleven benchmarked implementations, their representation, where they apply, and their role in the study.}
...
Roaring
  & 64-bit Roaring bitmap on the linearized bounding grid (CRoaring)
  & seven smaller structured scenarios
...
The benchmarked implementation targets C++23 and uses
\texttt{std::vector} for dynamic memory. It exposes throwing
\texttt{pos}/\texttt{at} and non-throwing \texttt{try\_pos}, although
no measured hot path throws. It also exposes the selection pipeline
used in Section~\ref{section:results-selection}: \texttt{IPT}
provides \texttt{select}, and \texttt{restrict} is measured by feeding
that view into the \texttt{std::from\_range} constructor. The hot
cuboid-search and cuboid-local translation kernels are the same as in
the deployed C++14 implementation integrated downstream into a
production geoscience pipeline. In that deployment, the existing
regular cases already use the IPT representation, and support for
irregular data is currently being added on a per-algorithm basis.
Relative to the benchmark harness, the downstream implementation
differs mainly in surrounding storage and error handling: it uses
single-allocation, memory-mapping-friendly buffers and an
...
Table~\ref{table:platforms} lists the 18 platform/compiler combinations,
grouped into nine hardware/OS rows.

\begin{table}[ht]
\centering
\caption{Benchmark platforms.}
\Description{A table listing nine hardware and operating-system rows that together yield eighteen benchmark platform and compiler combinations. Each row includes the processor model, operating system, kernel, and the available GCC and Clang versions.}
...
The cache-selection plot instead aggregates all 18 platform/compiler
...
Two workload families define the core construction/query suite, for 62
workload shapes in total. The auxiliary storage and hit-vs-miss
\texttt{try\_pos} benchmarks reuse the seven smaller structured
scenarios described below, so they add no new geometries.
...
($\kappa$ for IPTs), four point-query timings, and a selection/
restriction sweep.
...
queries in the main construction/query suite and at $10^4$ in the heavy
cache-independent and storage benchmarks.

For selection, each implementation is queried with 32 random axis-aligned
sub-cuboids at each relative window volume
$d \in \{0.001, 0.01, 0.1, 1.0\}$, and the benchmark records
\texttt{select\_d*} and \texttt{restrict\_d*} latency together with the
yielded cuboid and point counts.
...
30 repetitions, the ten structured 3D multiple-survey scenarios and the
optional 5D stress scenario use 5, and the storage and hit-vs-miss
\texttt{try\_pos} side benchmarks use 3.
...
drops the operating system's page cache for that file
...
\texttt{mmap}, recording the load duration. Sixth, it issues one
\texttt{pos}, one \texttt{at}, and one \texttt{select} call against
the freshly mapped instance, with no prior warm-up, to capture the
cold first-response latency that a live-loaded process would observe.

Seventh, it issues the same
random \texttt{pos}/\texttt{at} samples and the same \texttt{select}
sweep used in the regular benchmark, together with evenly spaced
\texttt{pos}/\texttt{at} loops capped at $10^4$ queries, all executed
through the mmap-backed instance, so that steady-state throughput on the
file-backed layout is
...
On all 62 core workload shapes the two construction algorithms produce
identical IPTs (same $\kappa$, same footprint, same entries). They
differ only on the crafted corner-steal family from
Section~\ref{section:construction}, where combine attains the optimum
and merge realises the proven $(d+1)/2$ gap.

Construction time does
vary: after averaging over repetitions within each
(platform, workload-shape) cell, the merge-versus-combine ratio has
median ${\sim}1.09\times$ across the full 18-platform suite. On the
ten structured multiple-survey scenarios the 90th percentile is
${\sim}1.26\times$ and the maximum ${\sim}1.34\times$, with larger
swings confined to the smallest random subsets where fixed overhead
dominates.

We therefore use greedy-plus-merge for the detailed figures
and retain greedy-plus-combine as the proven-optimal constructor, an
artifact variant, and a regression check exercised by
\texttt{make verify} on the crafted corner-steal inputs.

\paragraph{Cache configuration.}
All 16 cache configurations are benchmarked across all 18
platform/compiler combinations and all 62 core workload shapes.
...
\texttt{at} lever: \texttt{c0r0e1l0} reduces geometric-mean
\texttt{at} time to $0.499\times$ of the uncached baseline at only
$1.10\times$ memory. For \texttt{pos}, the ruler-size cache
(\texttt{r}~flag) is dominant and the least-upper-bound cache
(\texttt{l}~flag) adds further improvement: \texttt{c0r1e1l1} reaches
$0.907\times$ \texttt{pos} time and $0.429\times$ \texttt{at} time,
but grows to $1.70\times$ the minimal-footprint \texttt{c0r0e0l0}.

We
nevertheless choose \texttt{c0r1e1l0} as the default because it keeps
metadata to $1.40\times$ while still delivering $0.951\times$
\texttt{pos}, $0.438\times$ \texttt{at}, and $1.172\times$
...
\caption{Top: greedy-plus-merge geometric-mean construction,
  \texttt{pos}, and \texttt{at} ratios relative to \texttt{c0r0e0l0},
  aggregated over all 18 platform/compiler combinations and all 62 core
  ...
  distribution.}
\Description{A two-panel cache-configuration summary. The top panel shows grouped bars for greedy-plus-merge construction time, pos time, and at time ratios relative to c0r0e0l0 across all sixteen cache configurations. The bottom panel shows the corresponding memory ratios, with the configurations ordered from lower to higher memory cost. The labels for c0r1e1l0 and c0r1e1l1 are bold to highlight the recommended default and the query-optimized frontier point.}
\label{figure:at-cache}
\end{figure}

Pairwise comparisons of the 16-configuration ranking across the 18
platform/compiler combinations remain positive for the query metrics:
on greedy-plus-merge the median Spearman correlation is
$\rho = 0.88$ for \texttt{pos} and $\rho = 0.88$ for \texttt{at}, with
minima $0.24$ and $0.62$.
...
selected.
The weaker \texttt{pos} correlations are concentrated in a few outlier
platforms.
...
Construction ranks are less stable (median $\rho = 0.54$,
minimum $-0.34$), so the detailed construction numbers below are tied
to the reference platform and should be read as representative
measurements rather than as a universally fixed total ordering.
We use AMD EPYC~9454 (Rocky~10.1 / GCC~14.3.1) as that reference.

\section{Experimental Results}
\label{section:results}

Unless a sentence or caption explicitly says \emph{all 18
platform/compiler combinations} or \emph{platform-independent}, the
...
that \texttt{GreedyPlusCombine} recovers the optimum while
\texttt{GreedyPlusMerge} realizes the proven $(d+1)/2$ gap.

\subsection{Construction Performance}
\label{section:results-construct}

Both the IPT constructors and all applicable baselines are benchmarked
on lexicographically sorted input, matching the precondition of the
optimality proof in Section~\ref{section:construction} and the scope
statement in Section~\ref{section:discussion}. We therefore exclude the
cost of producing that ordering on every side. Pipelines that start
from an unordered point list pay the same sort cost for every format,
so the build-time ratios reported here are unchanged and the absolute
amortization points shift uniformly once that shared cost is charged.

On the EPYC~9454 reference platform, greedy-plus-merge construction
reaches $46.2{-}47.6$\,Mpoint/s on regular grids.
On the structured multiple-survey scenarios it achieves
$36.2{-}46.1$\,Mpoint/s, or $21.7{-}27.7$\,ns/point, again in a
comparatively narrow band despite the varying block layouts.
Figure~\ref{figure:construct-multi-survey} shows that this is not a
construction advantage over the non-IPT baselines. Across the four
cache-independent baselines available on all nine structured scenarios,
\texttt{SortedPoints} is fastest in geometric mean and builds in
$4.0{-}19.3$\,ns/point, while \texttt{LexRun}, \texttt{RowCSR-k1}, and
\texttt{RowCSR-k2} range over $2.4{-}19.9$, $4.1{-}18.8$, and
$4.5{-}12.3$\,ns/point respectively. On the seven smaller structured
scenarios where the occupancy-grid baselines are benchmarked,
\texttt{DenseBitset}, \texttt{BlockBitmap}, and \texttt{Roaring} build
in about $19.4{-}19.8$, $5.6$, and $10.2{-}11.7$\,ns/point, with
\texttt{BlockBitmap} the fastest builder on that covered subset.
Relative to these baselines, the IPT is slower by geometric-mean
factors of $2.6\times$ against \texttt{SortedPoints}, $2.5\times$
against \texttt{LexRun}, $3.5\times$ against \texttt{RowCSR-k1}, and
$3.9\times$ against \texttt{RowCSR-k2}, across all nine scenarios,
and by $1.29\times$, $4.46\times$, and $2.37\times$ against
\texttt{DenseBitset}, \texttt{BlockBitmap}, and \texttt{Roaring} on the
seven smaller scenarios. The cost is dominated by cuboid discovery and
merging.

For the later rank/unrank discussion we keep \texttt{SortedPoints} as
the amortization reference, because it is the general exact sorted-input
baseline used throughout the query sections. At the geometric-mean
rates on the structured scenarios (${\sim}42$\,Mpoint/s construction,
${\sim}9.1$\,Mpos/s \texttt{pos}), the $2.6\times$ build gap to
\texttt{SortedPoints} amounts to an extra ${\sim}15$\,ns per input
point. The $2.25\times$ \texttt{pos} speedup over \texttt{SortedPoints}
saves ${\sim}138$\,ns per query. Under the same sorted-input
assumption, the IPT therefore recovers that extra build cost after
approximately $N/9$ \texttt{pos} queries, where $N$ is the number of
input points. This break-even varies across platforms and structured
scenarios. Over the full $18\times 10$ platform-by-scenario matrix for
the structured multiple-survey family, after averaging within each
cell, $25$ cells already show no extra build cost for the IPT, and
among the remaining $155$ cells the 10th--90th percentile band is about
$0.011N$ to $0.225N$ \texttt{pos} queries. Workloads that issue about
one \texttt{pos} query per nine stored points therefore cross that
break-even within the first pass. An additional input sort shifts both
sides equally. Figure~\ref{figure:construct-multi-survey} shows the
per-point construction time on the nine structured multiple-survey
scenarios for the practical IPT and all applicable non-IPT baselines.
Figure~\ref{figure:construct-vs-density} shows construction time
as a function of density for the four grid sizes.

\begin{figure}
\centering
\resizebox{0.96\linewidth}{!}{\input{generated/construct-multiple-survey.tex}}
\caption{Construction time per point on the multiple-survey
  scenarios for the practical IPT and all applicable non-IPT baselines
  (\texttt{c0r1e1l0}, EPYC~9454 reference platform, with error bars
  spanning the 10th-90th percentile across the 5 repetitions).
  \texttt{DenseBitset}, \texttt{BlockBitmap}, and \texttt{Roaring} are
  absent on the two largest 3D scenarios because those baselines are
  only benchmarked on the seven smaller structured cases.}
\Description{A bar chart comparing practical IPT construction time per point against all applicable non-IPT baselines across the nine structured multiple-survey scenarios on the EPYC 9454 reference platform, with 10th-to-90th-percentile error bars across the 5 repetitions. DenseBitset, BlockBitmap, and Roaring appear only on the seven smaller scenarios.}
...
\caption{IPT construction time per point vs.\ point-set density
  ...
  EPYC~9454 platform, with error bars spanning the 10th-90th percentile across
  the 30 seeds).}
...
The cuboid counts and memory ratios in this subsection are
platform-independent because they depend only on the input geometry and
the chosen cache configuration, not on the benchmark machine.
...
Among the nine plotted multiple-survey scenarios, cuboid counts range
from $\sim\!2\,100$ to $\sim\!137\,000$. The larger 49-block 3D
outlier (\texttt{multiple-survey-12-fortynine}) reaches
$\sim\!1.7\!\times\!10^{6}$.
The largest 3D count still occurs in the 7-large 2D layout, whereas the
...
memory ratio (IPT\,/\,SortedPoints) is $0.088$
for \texttt{c0r1e1l0} and $0.063$ for the minimal
configuration \texttt{c0r0e0l0},
i.e.\ the selected default uses about $9\%$ of the sorted-points
footprint on those structured layouts.
For the sparsest random subsets ($1\%$) the high $\kappa$ can
push the IPT to ${\sim}\!3.3\times$ the sorted-points size with
\texttt{c0r1e1l0}, whereas the minimal-footprint \texttt{c0r0e0l0} reduces
that worst case to ${\sim}\!2.4\times$.

\begin{figure}
\centering
\input{generated/kappa-vs-density.tex}
\caption{Normalized cuboid count $\kappa/N$ as a function of
  point-set density for four grid sizes (platform-independent 30-seed
  mean, with error bars spanning the 10th-90th percentile across the 30
  seeds).}
...
$0.97\times$ that of the \texttt{Grid} baseline (i.e.\ the IPT is
within $3\%$ of the ideal pure-divmod cost) and the geometric-mean
\texttt{at} time is $0.97\times$. Across all 18
platform/compiler combinations, the worst per-platform geometric mean
on the same four regular grids is $1.01\times$ for \texttt{pos} and
$1.36\times$ for \texttt{at}.
...
\texttt{pos} to be $10{-}39\times$ faster than \texttt{SortedPoints}
(which performs a full \texttt{lower\_bound} over all $n$ points).
On the regular grids the IPT reaches $175{-}180$\,Mpos/s.
On the multiple-survey scenarios the advantage is
${\sim}2.25\times$ (geometric mean), with the IPT reaching
$1.5{-}22.3$\,Mpos/s.
As random-subset density drops, $\kappa$ rises and the IPT's
advantage shrinks: at $1\%$ and $50\%$ density the IPT is still
slower on the $100{\times}100{\times}100$ grid (time ratios $1.19$ and
$1.43$), the crossover occurs between $75\%$ and $90\%$ density
(ratios $1.16$ and $0.82$), and by $99\%$ density the IPT is already
about $1.9\times$ faster owing to the drop in~$\kappa$.
...
\texttt{RowCSR-k2} and \texttt{LexRun} are slower on both queries, and
on the seven smaller structured scenarios the occupancy baselines
remain two to three orders of magnitude slower despite bit-level
storage.

\begin{figure}
\centering
\resizebox{0.96\linewidth}{!}{\input{generated/pos-multiple-survey.tex}}
\caption{Absolute \texttt{pos} performance on the multiple-survey
  scenarios (\texttt{c0r1e1l0}, EPYC~9454 reference platform, with
  error bars spanning the 10th-90th percentile across the 5
  repetitions). Only algorithms whose 10th-90th-percentile range
  stays within 250~ns/\texttt{pos} are drawn for a given scenario.
  }
\Description{A bar chart comparing absolute pos query performance for IPT, SortedPoints, DenseBitset, BlockBitmap, Roaring, LexRun, RowCSR-k1, and RowCSR-k2 across the nine structured multiple-survey scenarios on the EPYC 9454 reference platform, with 10th-to-90th-percentile error bars across the 5 repetitions. Only series whose plotted range stays within 250 ns per pos are drawn for a given scenario. Algorithms omitted from the bars remain listed in the legend.}
...
\caption{\texttt{pos} time ratio (IPT / SortedPoints) vs.\ density
  (\texttt{c0r1e1l0}, EPYC~9454 platform, 30-seed geometric mean, with
  error bars spanning the 10th-90th percentile across the 30 seeds).}
...
On regular grids the sampled \texttt{at\_random} benchmark shows that
the IPT is $1.6{-}6.6\times$ slower than
\texttt{SortedPoints}, but it stays within ${\sim}3\%$ of the ideal
regular-grid baseline (\texttt{Grid}) because the selected caches
leave the one-entry search near-trivial.
On random subsets the gap widens because $\kappa$ determines the
binary-search depth.
This is the expected cost of trading per-point storage for
per-cuboid metadata.
Figure~\ref{figure:at-multi-survey} shows the absolute \texttt{at}
performance on the nine structured multiple-survey scenarios.
Across the full ten-scenario multiple-survey family, the IPT reaches
$2.3{-}32.0$\,M\texttt{at}/s and is $17.8\times$ slower than
\texttt{SortedPoints} in geometric mean.
...
\begin{figure}
\centering
\resizebox{0.96\linewidth}{!}{\input{generated/at-multiple-survey.tex}}
\caption{Absolute \texttt{at} performance on the multiple-survey
  scenarios (\texttt{c0r1e1l0}, EPYC~9454 reference platform, with
  error bars spanning the 10th-90th percentile across the 5
  repetitions). Only algorithms whose 10th-90th-percentile range
  stays within 250~ns/\texttt{at} are drawn for a given scenario.
  Omitted algorithms remain in the legend.}
\Description{A bar chart comparing absolute at query performance for IPT, SortedPoints, DenseBitset, BlockBitmap, Roaring, LexRun, RowCSR-k1, and RowCSR-k2 across the nine structured multiple-survey scenarios on the EPYC 9454 reference platform, with 10th-to-90th-percentile error bars across the 5 repetitions. Only series whose plotted range stays within 250 ns per at are drawn for a given scenario. Algorithms omitted from the bars remain listed in the legend.}
...
$O(d)$ per lexicographic tuple comparison, while $d\,t$ is the
per-candidate intersection cost and $d\,r$ is the per-result yield
cost. This matches
...
To validate this decomposition we instrumented the same eight non-IPT
baselines used across the query experiments (\textsc{Grid},
\textsc{SortedPoints}, \textsc{DenseBitset}, \textsc{BlockBitmap},
\textsc{Roaring}, \textsc{LexRun}, \textsc{RowCSR-k1},
\textsc{RowCSR-k2}) and the three IPT configurations
(\textsc{Regular}, \textsc{GreedyPlusCombine},
\textsc{GreedyPlusMerge}) with a uniform \texttt{select} method that
emits a lazy stream of \texttt{Cuboid<D>} fragments covering exactly
the points in the query window. The point-level baselines emit one
singleton cuboid per matching point, \textsc{LexRun} emits one cuboid
per surviving lex-run, and \textsc{Grid} and the IPT variants emit at
most one fragment per intrinsic cuboid.

The benchmark binary, when
...
that the ten non-reference implementations yield identical point sets
to the \textsc{SortedPoints} reference after enumeration.

\paragraph{What the \texttt{restrict} numbers actually compare.}
For every implementation, including the non-IPT baselines, the
reported \texttt{restrict} pipeline is the same two-stage recipe:
\emph{select-then-IPT-rebuild}. For IPT this matches the native
operator $\textsc{restrict}(Q) = \mathrm{IPT}(\textsc{select}(Q))$.
For the non-IPT baselines it is not a layout-native restriction
operator, because those layouts would naturally produce smaller
instances of themselves rather than an IPT. The numbers therefore
measure how well each layout's selection feeds an IPT rebuild, which
is the shared workload the IPT advertises.

The full 18-platform/compiler sweep emits \texttt{select\_d*} and
\texttt{restrict\_d*} measurements for every algorithm and scenario,
so the asymptotic claims above are checked across the same matrix as
\texttt{at} and \texttt{pos}. The measured \texttt{select} time scales
linearly with the number of yielded fragments, in line with the
$O(d\,r)$ term. On the EPYC~9454 reference platform at a $d=0.01$
window, \texttt{restrict} is only about $2.2\%$ above bare
\texttt{select} for the IPT in geometric mean across the nine plotted
structured scenarios, so restriction adds no material asymptotic
overhead beyond enumeration.

On the same workloads the IPT's
\texttt{select} is geometric-mean $6.7\times$ faster than
\texttt{SortedPoints}, $5.2\times$ faster than \texttt{RowCSR-k1},
$63\times$ faster than \texttt{RowCSR-k2}, and $309\times$ faster
than \texttt{LexRun}. On the seven smaller scenarios where they are
tractable, \texttt{DenseBitset}, \texttt{BlockBitmap}, and
\texttt{Roaring} are $10.6\times$, $6.8\times$, and $14.6\times$
slower. The corresponding \texttt{restrict} ratios stay within about
$\pm 30\%$ because restriction adds only the from-range rebuild. The
gaps widen at smaller $d$ and shrink at $d = 1$ as the benchmark
becomes yield-bound.
...
each (cache configuration, seed) cell it records serialize time,
bytes-on-disk, cold-load via \texttt{mmap} after dropping the page
cache, first-response latencies for \texttt{pos}/\texttt{at}/
\texttt{select}, and steady-state mmap-backed throughput using the
same loops as Sections~\ref{section:results-pos}--%
\ref{section:results-selection}. The goal is to test whether these
flat layouts can be written once, mapped back without reconstruction
or pointer fix-up, and then deliver on-disk size and throughput that
track the in-memory results. The \texttt{c0r0e0l0} versus
\texttt{c0r1e1l0} pairing exposes the size/throughput trade-off from
Section~\ref{section:selection}. The per-scenario tables and figure
are extracted from \texttt{storage-c0r0e0l0.txt} and
\texttt{storage-c0r1e1l0.txt}, emitted by
\texttt{make benchmark-storage}.
...
EPYC~9454 reference platform. Three points matter. First, the IPT file
is $1.66$\,MB at \texttt{c0r1e1l0} and $1.19$\,MB at
\texttt{c0r0e0l0}, that is $5.2\times$ and $7.2\times$ smaller than
\texttt{SortedPoints} respectively, mirroring the in-memory footprint.
\textsc{Roaring}, \textsc{DenseBitset}, and \textsc{BlockBitmap} are
smaller still in absolute bytes, but pay for that compactness on query
throughput. Second, the IPT's cold first-response latencies after
\texttt{mmap} are $9.5\,\mu$s for \texttt{pos}, $3.8\,\mu$s for
\texttt{at}, and $7.6\,\mu$s for \texttt{select}, showing that
mapping is not followed by reconstruction or pointer fix-up. Third,
the steady-state mmap-backed throughput ($7.0$\,Mpos/s and
$15.9$\,Mat/s) stays in the same band as the in-memory measurements,
so mmap is a viable runtime substrate rather than only a load path. As
in Section~\ref{section:selection}, \texttt{c0r0e0l0} trades space for
a small \texttt{select}-latency penalty.
...
\caption{Persistence summary on the EPYC~9454 reference platform
  ...
  \texttt{c0r1e1l0}, greedy-plus-merge). Each metric is reported with its value and
  ratio relative to the default \textsc{IPT} value. Bold values mark
  the best entry in each metric column. Bold ratios indicate results
  strictly better than \textsc{IPT}.}
\Description{Table summarising bytes on disk, bytes per point, cold
  first-response latency for pos / at / select, and steady-state
  mmap-backed pos and at throughput, per algorithm, geometric mean
  across the seven smaller structured multiple-survey scenarios
  on the EPYC 9454 reference platform, with a three-column byte group
  for total size, bytes per point, and ratio to the default IPT value,
  and ratio columns for the remaining metrics.}
...
\caption{Hit-vs-miss \texttt{try\_pos} cost (geometric mean over the
  seven smaller structured multiple-survey scenarios, averaged across
  three seeds, on the
...
unique 5D points. We collected this scenario on all 18
platform/compiler combinations with the default \texttt{c0r1e1l0}
cache configuration. The aggregate appears
in Table~\ref{table:stress-5d-summary}.

\begin{table}[ht]
\centering
\caption{Aggregate \texttt{multiple-survey-9-5d} measurements
  (greedy-plus-merge, \texttt{c0r1e1l0}).
  The upper table lists the platform-independent cuboid count, IPT
  byte size, and bytes per point. The lower table reports the geometric mean and the
  10th--90th percentile across all 18 platform/compiler combinations.}
\Description{Table of aggregate measurements for the
  multiple-survey-9-5d 5D stress scenario. The upper tabular lists the
  platform-independent cuboid count, IPT byte size, and bytes per
  point. The lower
  tabular summarises construction rate, pos and at throughput, and
  select/restrict latency at two density levels, with geometric mean
  and 10th-to-90th percentile across 18 platform/compiler
  combinations.}
...
\begin{table}[ht]
\centering
\caption{IPT / \texttt{SortedPoints} time ratios on
  \texttt{multiple-survey-9-5d} (\texttt{c0r1e1l0}). Values below 1
  favor the IPT. The table reports the geometric mean and the
  10th--90th percentile across all 18 platform/compiler combinations.}
\Description{Table of IPT over SortedPoints time ratios on the
  multiple-survey-9-5d 5D stress scenario. Rows cover construction,
  pos_random, pos_all, at_random, at_all, and select and restrict at
  relative window sizes d equals 0.01 and d equals 0.1. Values below
  one favor the IPT, and the table reports the geometric mean and the
  10th-to-90th percentile across 18 platform/compiler combinations.}
\label{table:stress-5d-ratio-summary}
\input{generated/stress-ratio-summary.tex}
\end{table}

Three observations carry over from the lower-dimensional results. The
cuboid count is only $\kappa = 2154$, so the IPT metadata fits in
$379\,104$ bytes ($\approx 370\,$KiB) for $9.2\times 10^7$ points,
that is about $42\,800$ points per cuboid record. The
\texttt{at}/\texttt{pos} gap persists at $d=5$, and \texttt{select}
and \texttt{restrict} stay in the $10$--$10^3\,\mu$s range up to a
$d=0.1$ relative window, confirming that the cuboid-algebra path still
scales with structure rather than with $N$.

Table~\ref{table:stress-5d-ratio-summary} reports IPT /\
\texttt{SortedPoints} time ratios on the same corpus. The qualitative
picture is unchanged: construction remains more expensive for the IPT
($1.68\times$ in geometric mean), \texttt{pos} remains clearly faster
($0.14$ for \texttt{pos\_random} and $0.048$ for \texttt{pos\_all}),
\texttt{at} remains slower ($6.91\times$ for \texttt{at\_random} and
$2.05\times$ for \texttt{at\_all}), and the cuboid-algebra
\texttt{select}/\texttt{restrict} path remains three to four orders of
magnitude faster than \texttt{SortedPoints}
($2\times 10^{-4}$ to $4\times 10^{-4}$).

We keep this 5D case separate from
Sections~\ref{section:results-construct}--\ref{section:results-pos-mix}
because those plots and tables are defined over fixed 3D suites,
whereas \texttt{multiple-survey-9-5d} serves here as a higher-dimensional,
cross-platform stress check. Tables~\ref{table:stress-5d-summary}
and~\ref{table:stress-5d-ratio-summary} should therefore be read as
evidence that the structured-regime properties from
Sections~\ref{section:results-pos}--\ref{section:results-selection}
persist when $d$ and $N$ increase together, not as another point folded
into the earlier 3D aggregates.
...
savings and clear $\operatorname{pos}$ gains over \texttt{SortedPoints}.
...
$\operatorname{at}$ and can also erase the $\operatorname{pos}$ advantage. The governing
...
The experiments match the complexity analysis: construction is linear
in~$N$, $\kappa$ collapses to~1 on (strided-)regular inputs and grows
sub-linearly on random subsets, and both $\operatorname{at}$ and $\operatorname{pos}$
show the expected dependence on~$\kappa$.

Across the non-crafted workloads in the core suite, the two
construction algorithms produced identical decompositions. This does
not contradict the counterexamples from
Section~\ref{section:construction}. It shows that corner-steal
patterns are rare in this workload family. The remaining timing gaps
are mostly confined to the smallest random-grid subsets, where fixed
overhead dominates, so greedy-plus-merge is the simpler default for
the detailed figures. We retain greedy-plus-combine as the
proven-optimal constructor and as a regression check via
\texttt{make verify}.
...
fast $\operatorname{pos}$. When fragmentation drives $\kappa$ toward~$N$, the
...
The multiple-survey benchmarks only hint at this effect: the first six
structured scenarios are effectively 2D-in-3D, whereas the last three
vary in every dimension. If the common third-dimension extent in the
first family were increased from $1$ to $1000$, the IPT metadata would
remain unchanged, while \texttt{SortedPoints} would store $1000$ times
as many points and pay correspondingly more for $\operatorname{pos}$.
...
One scope limit matters throughout: the construction algorithms are
...
on production geometries, especially outside geoscience.
...
\texttt{Makefile} drives the full pipeline:
\begin{itemize}
  ...
  \item \texttt{make benchmark} runs the cache-dependent IPT
    ...
    cache-independent baselines once per platform, and output goes to
...
names. The header-only IPT library lives in
...
provided in \texttt{CITATION.cff}.
...
\path{third_party/CRoaring/} and built once into the per-platform
...
array-based structure. The paper gives linear-time construction
procedures based on greedy repair. In particular,
greedy-plus-combine achieves the minimum cuboid count in $O(dN)$ time
on sorted input.

Empirically, the IPT is most effective when irregular data retains
substantial cuboid structure and the workload is
\texttt{pos}-, selection-, or restriction-heavy. On the structured
survey layouts it keeps about $9\%$ of the \texttt{SortedPoints}
memory, improves \texttt{pos} by about $2.25\times$, and provides a
closed-form geometric restriction operator, while staying close to the
regular-grid fast path with \texttt{c0r1e1l0}. On
\texttt{at}-heavy traversals of the same layouts it is slower than a
sorted-points table. Across the non-crafted workloads,
greedy-plus-combine and greedy-plus-merge produced the same
decompositions, so combine's practical role here is proof-backed
optimality and regression checking rather than additional measured
speed or compression. The cache sweep confirms \texttt{c0r1e1l1} as
the query-optimised frontier and \texttt{c0r1e1l0} as the broader
default under a $40\%$ metadata-overhead budget.
...
We prove that greedy-plus-combine always produces a decomposition with
the minimum number of cuboids.
...
Let $(C^*_0, \ldots, C^*_{\kappa_{\text{opt}}-1})$ be a
minimum-cuboid decomposition of~$D$, chosen so that $C^*_0$ is as
large as possible. We show that greedy-plus-combine reconstructs
$C^*_0$ and never modifies it afterward.

The proof has three stages. First, we show that the algorithm
reconstructs the first optimal cuboid~$C^*_0$. Second, if the greedy
scan extends past~$C^*_0$, we show that the overshoot can only be a
corner-steal in~$\dim_0$ and that the combine step repairs it
immediately. Third, once $C^*_0$ has appeared, we show that it stays
fixed. In the no-steal branch, canonical maximality rules out merges
and lexicographic separation rules out combines. In the post-repair
branch, a strict gap in~$\dim_0$ isolates $C^*_0$ from all later
cuboids.

\paragraph{Recovery of~$C^*_0$.}
Consider the prefix of the input consisting exactly of the points
of~$C^*_0$. Since $C^*_0$ is one cuboid, the one-cuboid recovery
result from Appendix~\ref{proof:one-cuboid-recovery} shows that
greedy-plus-merge would finish this prefix as one cuboid. It remains
to check that the extra combine step cannot change that outcome.

The one-cuboid recovery proof processes $C^*_0$ slice by slice in
dimension~0. While one slice is being processed, the cuboid list
consists of one accumulated cuboid~$A$ from earlier slices together
with the cuboids currently created inside the active slice. A combine
involving $A$ cannot fire, because the bounding box of the last two
cuboids lies entirely in the current slice, whereas~$A$ lies in
earlier slices. Their deficit is therefore disjoint from~$A$. A
combine using only cuboids from the current slice also stays inside
that slice. After deleting the fixed $\dim_0$ coordinate, the slice
run becomes exactly a greedy-plus-combine run on the
$(d{-}1)$-dimensional one-cuboid input defined by that slice. By the
secondary induction on~$d$, that lower-dimensional run reconstructs
the slice as one cuboid. Hence each slice is finished correctly, and
adjacent finished slices merge exactly as in one-cuboid recovery. When
the last point of~$C^*_0$ has been processed, the current first cuboid
is $C^*_0$.
...
Let $p_1 = \operatorname{glb}(C^*_1)$. For $C^*_0 \cup \{p_1\}$ to
be a cuboid, the new point can extend exactly one ruler of~$C^*_0$.
Therefore every other ruler of~$C^*_0$ has length~$1$, so $C^*_0$ is
a column in some dimension~$m$. Moreover, $C^*_1$ cannot be a column
in that same dimension with the same fixed coordinates in all other
dimensions, because then $C^*_0 \cup C^*_1$ would itself be a cuboid
and the chosen decomposition would not be optimal.
...
Partition $C^*_1$ into \emph{fibres}, where a fibre is the set of
points that agree in every coordinate except~$\dim_{m'}$. These fibres
arrive consecutively in lexicographic order. The first fibre contains
$p_1$. Since $p_1$ has already been absorbed into~$C_0$, the greedy
scan collects the remaining $n'_{m'} - 1$ points of that fibre into a
cuboid~$C_1$. The next fibre is complete, so it forms a cuboid~$C_2$
of length $n'_{m'}$ in~$\dim_{m'}$. Because $C^*_1$ is a cuboid,
every fibre uses the same ruler in~$\dim_{m'}$, so the completed
first fibre $C_1 \cup \{p_1\}$ and the next fibre $C_2$ both have
length $n'_{m'}$ in that dimension. The two cuboids cannot merge,
because their rulers in~$\dim_{m'}$ have different lengths.

Let $K = \operatorname{bbox}(C_1 \cup C_2)$. This cuboid contains
exactly the two fibres together with the single missing point~$p_1$.
Hence the deficit of~$K$ is precisely $\{p_1\}$, which lies in~$C_0$.
Removing~$K$ from the extended column restores the original column, so
$C_0 \setminus K = C^*_0$ is again a cuboid. Therefore the combine
step replaces $(C_0, C_1, C_2)$ by $(C^*_0, K)$ and repairs the
earlier absorption of~$p_1$ as soon as the second fibre has been
processed.
...
Once the algorithm has a cuboid equal to~$C^*_0$, that cuboid is never
modified again. We treat the two configurations separately. In the
no-steal branch, $C^*_0$ may have arbitrary shape and the argument has
to work across all~$d$ dimensions. After a corner-steal repair,
$C^*_0$ is by construction a $\dim_0$ column whose $\dim_0$ range is
strictly below every subsequent input point.

\emph{No-steal branch.}
If no steal occurs, $C_0 = C^*_0$ once the prefix corresponding
to~$C^*_0$ has been processed, and the next input point is
$p_1 = \operatorname{glb}(C^*_1)$. We rule out merge and combine
separately. Merge is excluded by the canonical-decomposition choice
of~$C^*_0$. Combine is excluded by lexicographic separation.

\emph{No merge.}
Recall that the optimal decomposition
$(C^*_0, \ldots, C^*_{\kappa_{\mathrm{opt}}-1})$ was chosen, among
all minimum-$\kappa$ decompositions of~$D$, to maximise $|C^*_0|$
(canonical decomposition). Suppose a later cuboid~$C_l$ ($l \geq 1$)
merges with~$C^*_0$ during the run, so that $C^*_0 \cup C_l$ is a
cuboid. By the strong induction hypothesis on~$N$ applied to
$D \setminus (C^*_0 \cup C_l)$, greedy-plus-combine produces a
minimum-cuboid decomposition $(C''_0, \ldots)$ of that residual. Then
$(C^*_0 \cup C_l, C''_0, \ldots)$ is a minimum-cuboid decomposition
of~$D$ whose first cuboid strictly exceeds~$C^*_0$ in size. That
contradicts the canonical-decomposition hypothesis.
...
This descent terminates because each such projection removes the first $j'{+}1 \geq 1$ coordinates, so the ambient dimension strictly drops from $d$ to $d - j' - 1 < d$ until the argument reaches the already-settled case $j \leq j'$.
...
Since $C_0 = C^*_0$ and is stable, the algorithm processes
$D' = D \setminus C^*_0$ independently. By strong induction on~$N$
($|D'| < |D|$), greedy-plus-combine produces a minimum-cuboid
decomposition of~$D'$. The total cuboid count is
$1 + (\kappa_{\text{opt}} - 1) = \kappa_{\text{opt}}$.
\end{proof}
