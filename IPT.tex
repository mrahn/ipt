...
sorted input. Benchmarks on eighteen x86 platform/compiler combinations
...
IPT reaches $175{-}180$\,Mpos/s on the tested regular grids while
staying within $3\%$ throughput of pure mixed-radix rank/unrank on
that regular case, uses about $8.8\%$ of the memory of a sorted-points
table on the structured multiple-survey layouts, and accelerates
\texttt{pos} by
$10{-}39\times$ on regular grids and by ${\sim}2.25\times$ on the
...
The IPT is strong on structured irregular workloads where data
with $N$ points decomposes into $\kappa\ll N$ strided-regular regions
and the dominant operations are \texttt{pos}, selection, or geometric
restriction. The IPT advantages narrow on \texttt{at}-heavy traversals
and disappears on highly fragmented subsets where $\kappa \approx N$.
...
with doubling growth, uses throwing \texttt{pos}/\texttt{at} together
with non-throwing \texttt{try\_pos} (no throw is hit on any measured
hot path), and exposes the selection pipeline
...
Two workload families define the core construction/query suite,
totaling 62 workload shapes. The auxiliary storage and hit-vs-miss
\texttt{try\_pos} benchmarks reuse only the seven smaller structured
scenarios described below, so they do not add further geometry shapes
to this count.
\emph{Grid scenarios:}
...
49-block outlier (\texttt{multiple-survey-12-fortynine}) is included
in Table~\ref{table:structured-scenarios} for completeness but is
discussed only in aggregate text so that this 3D outlier does not
dominate the plot scales.
...
On every one of the 62 core workload shapes the two construction
algorithms produce identical IPTs (same $\kappa$, same footprint, same
entries).
The two algorithms only diverge in the resulting decomposition on the
crafted corner-steal family from Section~\ref{section:construction},
where combine attains the optimum and merge realises the proven
$(d+1)/2$ gap.
Construction time, however, is not within a uniform ${\sim}1.1\times$
band in the current runs.
After averaging over repetitions within each
(platform, workload-shape) cell, the merge-versus-combine construction
ratio has median ${\sim}1.09\times$.
On the ten structured multiple-survey scenarios the 90th percentile is
${\sim}1.26\times$ and the maximum ${\sim}1.34\times$. The much larger
relative swings occur only on the smallest random grid subsets, where
fixed overhead dominates ns/point.
On the measured non-artificial suite, combine therefore serves mainly
as an optimality guarantee rather than as a source of different query
or compression behaviour.
...
reduces geometric-mean \texttt{at} time to $0.499\times$ of the uncached
...
improvement.
With both \texttt{r} and \texttt{l} enabled, \texttt{c0r1e1l1} reaches
$0.907\times$ of the uncached \texttt{pos} time while keeping
\texttt{at} near $0.429\times$.
For a more query-weighted objective, \texttt{c0r1e1l1} is therefore a
natural frontier point, but its metadata grows to $1.70\times$ the size of the
...
benefit: relative to \texttt{c0r0e0l0} it yields $0.951\times$
\texttt{pos}, $0.438\times$ \texttt{at}, $1.172\times$ construction
...
prefix, and the absolute amortization point shifts uniformly once a sort is
charged.

On the EPYC~9454 reference platform, greedy-plus-merge construction
reaches $43.5{-}48.0$\,Mpoint/s on regular grids.
On the structured multiple-survey scenarios it achieves
$36.3{-}46.2$\,Mpoint/s, again in a comparatively narrow band despite the
varying block layouts.
\texttt{SortedPoints} construction is essentially a memory copy
and reaches up to ${\sim}\!600$\,Mpoint/s on the smallest regular grid
(where it amounts to little more than a 24\,kB \texttt{memcpy}) and
$52{-}460$\,Mpoint/s on the structured scenarios, so the IPT is
${\sim}2.9\times$ more expensive to build on the structured workloads
...
(${\sim}42$\,Mpoint/s construction, ${\sim}9.1$\,Mpos/s \texttt{pos}),
the $2.9\times$ build overhead amounts to an extra ${\sim}16$\,ns per
input point. The $2.25\times$ \texttt{pos} speedup over
\texttt{SortedPoints} saves ${\sim}138$\,ns per query.
Under the same sorted-input assumption, the IPT therefore recovers its
build cost after approximately $N/9$ \texttt{pos} queries, where $N$ is
the number of input points.
Any workload that issues at least one query per nine stored points
...
\caption{Construction time per point on the multiple-survey
  ...
  error bars spanning the 10th-90th percentile across 30 seeds).}
...
\caption{Normalized cuboid count $\kappa/N$ as a function of
  ...
  platform, with error bars spanning the 10th-90th percentile across the 30
  seeds).}
...
On regular grids the IPT is $1.6{-}6.6\times$ slower than
...
performance on the nine structured multiple-survey scenarios.
On those structured scenarios the IPT reaches $2.3{-}32.0$\,M\texttt{at}/s
and is $18.1\times$ slower than \texttt{SortedPoints} in geometric mean.
...
most one fragment per intrinsic cuboid. The benchmark binary, when
compiled without \texttt{NDEBUG} (i.e.\ as exercised by
...
Section~\ref{section:operations-restriction}, whereas for the non-IPT baselines
...
only about $1.3\%$ above the bare \texttt{select} cost for the IPT in
...
At a $d = 0.01$ window on the structured multiple-survey
scenarios, the IPT's \texttt{select} is geometric-mean
$4.1\times$ faster than \texttt{SortedPoints},
$3.2\times$ faster than \texttt{RowCSR-k1},
$56\times$ faster than \texttt{RowCSR-k$d{-}1$}, and
$114\times$ faster than \texttt{LexRun}.
On the seven smaller scenarios where they are tractable,
\texttt{DenseBitset}, \texttt{BlockBitmap} and \texttt{Roaring} are
$10.5\times$, $6.9\times$ and $15.2\times$ slower than the IPT.
The corresponding \texttt{restrict} ratios fall within $\pm 30\%$ of
those values (e.g.\ $3.7\times$ for \texttt{SortedPoints} and
$2.9\times$ for \texttt{RowCSR-k1}), since restriction adds only the
...
quantities per (cache configuration, seed) cell. These are
...
\texttt{select} on the freshly mapped instance, with no warm-up, and
...
\paragraph{Results.}
Table~\ref{table:storage-summary} summarises the per-algorithm geometric
means across the seven smaller structured multiple-survey scenarios on
the EPYC~9454 reference platform. Three quantitative observations carry the
flat-layout claim. First, on these structured layouts the IPT file is
$1.66$\,MB at the \texttt{c0r1e1l0} default and $1.19$\,MB at the
minimal \texttt{c0r0e0l0} (a further $28\%$ reduction), confirming
that the on-disk size tracks the in-memory metadata footprint
reported in Section~\ref{section:results-summary}. The IPT file is
$5.2\times$ smaller than the corresponding \texttt{SortedPoints} file
at the default and $7.2\times$ smaller at the minimal configuration,
which matches the in-memory ratio band reported earlier.
\textsc{Roaring}, \textsc{DenseBitset} and \textsc{BlockBitmap}
remain smaller in absolute bytes (because they encode a packed bitmap
rather than per-cuboid metadata), but they pay for that compactness on
every query as the throughput columns of
Table~\ref{table:storage-summary} show. Second, the IPT's cold
first-response latency after \texttt{mmap} is $9.5\,\mu$s for
\texttt{pos}, $3.8\,\mu$s for \texttt{at} and $7.6\,\mu$s for
\texttt{select}~--- bounded by a handful of cold page faults plus a
$\log\kappa$ binary search and dominated by neither a reconstruction
nor a pointer-fix-up phase. Third, the steady-state \texttt{pos} and
\texttt{at} throughput on the mmap-backed instance ($7.0$\,Mpos/s and
$15.9$\,Mat/s on these structured layouts) is in the same band as the
in-memory numbers reported in
Section~\ref{section:results-pos}~--- mmap is therefore a viable
steady-state runtime substrate, not just a load mechanism. The
\texttt{c0r0e0l0} versus \texttt{c0r1e1l0} pairing also illustrates
the size/throughput trade-off discussed in
Section~\ref{section:selection}: dropping the in-memory caches saves
disk space at the cost of a small \texttt{select}-latency penalty,
because the cache fields would otherwise be stored once and amortised
over every future cold load.
...
\caption{Persistence summary on the EPYC~9454 reference platform
  ...
  \texttt{c0r1e1l0}). The bottom row reports the IPT file size with
  the minimal \texttt{c0r0e0l0} cache configuration. The byte group
  reports total size, bytes per point, and a shared ratio relative to
  the default \textsc{IPT} value, while the remaining metrics are
  followed by their ratio. \textsc{IPT} is
  greedy-plus-merge. Baseline acronyms match
  Section~\ref{section:results-pos}.}
\Description{Table summarising bytes on disk, bytes per point, cold
  ...
  across the seven seven smaller structured multiple-survey scenarios
...
{\setlength{\tabcolsep}{3pt}\footnotesize\input{generated/storage-summary.tex}}
\end{table}
...
To connect this 5D case back to the earlier 3D head-to-head results,
Table~\ref{table:stress-5d-ratio-summary} reports IPT /\
\texttt{SortedPoints} time ratios on the same corpus.
The qualitative picture matches the lower-dimensional sections:
construction remains more expensive for the IPT
($1.68\times$ in geometric mean), \texttt{pos} remains clearly faster
(time ratios $0.14$ for \texttt{pos\_random} and $0.048$ for
\texttt{pos\_all}), \texttt{at} remains slower
($6.91\times$ for \texttt{at\_random} and $2.05\times$ for
\texttt{at\_all}), and the cuboid-algebra \texttt{select} /\
\texttt{restrict} path remains three to four orders of magnitude faster
than \texttt{SortedPoints} (time ratios of
$2\times 10^{-4}$ to $4\times 10^{-4}$).

The raw stress files therefore do contain a head-to-head baseline view,
but we still keep this 5D case separate from
Sections~\ref{section:results-construct}--\ref{section:results-pos-mix}
because those plots and tables are defined over fixed 3D comparison
suites (or the seven-scenario storage/pos-mix subset), whereas
\texttt{multiple-survey-9-5d} is used here as a higher-dimensional,
cross-platform stress check rather than as one more member of those
suites.
Tables~\ref{table:stress-5d-summary}
...
Sections~\ref{section:results-pos}--\ref{section:results-selection}
persist when $d$ and $N$ are increased together, not as another data
point folded into the earlier 3D aggregates.
...
Across all 62 workload shapes, the two construction algorithms produced
identical decompositions. This does not contradict the
...
tested input family. As discussed in
Section~\ref{section:selection}, the remaining timing gaps are mainly
confined to the smallest random-grid subsets where fixed overhead
dominates ns/point. On non-artificial inputs the two algorithms are
interchangeable in result and close in performance, so we keep
...
IPT keeps about $9\%$ of the sorted-points memory on the tested
structured survey layouts, improves \texttt{pos} by about $2.25\times$
