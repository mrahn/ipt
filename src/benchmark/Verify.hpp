...
namespace verify
{
      ...
      ) -> void
  {
    ...
    auto const dynamic_programming
      { build_ipt<D, create::DynamicProgramming<D>>
          (std::span<Point<D> const> {source.points()})
      };
    ...
    if (!(dynamic_programming == expected))
    {
      throw std::runtime_error
        { std::format
            ( "{}: DynamicProgramming did not recover the expected optimal IPT"
            , label
            )
        };
    }
    if (dynamic_programming.number_of_entries() != source.optimal_entry_count())
    {
      throw std::runtime_error
        { std::format
            ( "{}: expected {} DynamicProgramming entries, got {}"
            , label
            , source.optimal_entry_count()
            , dynamic_programming.number_of_entries()
            )
        };
    }
    ...
    if ( 2 * greedy_merge.number_of_entries()
       != (D + 1) * dynamic_programming.number_of_entries()
       )
    {
      throw std::runtime_error
        { std::format
          ( "{}: GreedyPlusMerge lower-bound ratio is not (D+1)/2"
          , label
          )
        };
    }
    ...
    std::print
      ( "verify corner_steal D={} copies={} points={} optimum={} "
        "greedy-plus-merge={} dynamic-programming={}\n"
      , D
      , source.copies()
      , source.points().size()
  ...
  inline auto run_restrict() -> void
  {
    ...
    auto query_index {std::size_t {0}};
    constexpr auto random_query_count {std::size_t {200}};
    constexpr auto stencil_center_count {std::size_t {5}};
    constexpr auto distribution_query_sample_count {std::size_t {5}};

    auto check_query_case
      { [&]
          ( Cuboid<3> const& query
          , std::vector<Point<3>> const& reference
          ) -> void
        {
    ...
    for ( auto query_index {std::size_t {0}}
        ; query_index < random_query_count
        ; ++query_index
        )
    {
      auto build_one
      ...
      check_query_case (query, reference);
    }

    auto const stencil_centers
      { evenly_spaced_sample
          ( std::span<Point<3> const> {points}
          , stencil_center_count
          )
      };

    for (auto const& spec : restrict_stencil_specs<3>())
    {
      for (auto const& center : stencil_centers)
      {
        auto const query
          { make_stencil_query<3>
            ( world
            , center
            , spec
            )
    ...
    for (auto const& spec : restrict_distribution_specs())
    {
      auto const distribution_queries {make_distribution_queries (world, spec)};
      auto const sampled_queries
        { evenly_spaced_sample
            ( std::span<Cuboid<3> const> {distribution_queries}
            , distribution_query_sample_count
            )
        };

      for (auto const& query : sampled_queries)
      {
        auto const reference {materialize_cuboid_points (query)};
    ...
    check_query_case (outside_empty_query, empty_reference);
    check_query_case (partial_overlap_empty_query, empty_reference);
    check_query_case (full_overlap_empty_query, empty_reference);
