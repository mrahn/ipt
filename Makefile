...
# label comes from the first /proc/cpuinfo model line. The OS label
# comes from /etc/os-release. The kernel label comes from `uname -r`.
# The compiler label comes from the first line of `$(CXX) --version`.
# The same tag scopes build/src-layout/<platform>/benchmark/ so that
...
DP_QUALITY_SRC  := $(BENCHMARK_SRC_DIR)/dynamic-programming-quality.cpp
DP_QUALITY_BIN  := $(EXTRACT_BIN_DIR)/dynamic-programming-quality

RESULT_DIRS := $(sort $(patsubst %/,%,$(wildcard results/os_*/)))
...
.PHONY: paper verify benchmark benchmark-grid benchmark-cache-sweep \
	benchmark-storage benchmark-pos-mix benchmark-restrict benchmark-5D \
	plots clean distclean help \
        benchmark-build \
        regenerate-dynamic-programming-quality
...
help:
	...
	@echo '  plots                            Build TSVs, figure assets, and'
	@echo '                                   generated summary tables only.'
	@echo '  regenerate-dynamic-programming-quality'
	@echo '                                   Recompute generated/dynamic-programming-quality.tex'
	@echo '                                   (about 13 minutes on the reference machine).'
	@echo ''
	...
	@echo '                                   including storage, pos-mix,'
	@echo '                                   restrict, and 5D stress runs.'
	...
	@echo '                                   combination.'
	@echo '  benchmark-restrict               Dedicated stencil and distribution'
	@echo '                                   restrict workloads.'
...
# Scenario selection and cache combinations are encoded in the targets
# and compiler definitions.
#
BENCHMARK_DIR        := $(CURDIR)/build/src-layout/$(PLATFORM_TAG)/benchmark
...
# touching them invalidates the corresponding executable.
BENCHMARK_DEPENDENCY_GOALS := verify verify-% benchmark benchmark-% benchmark-build
ifneq ($(filter $(BENCHMARK_DEPENDENCY_GOALS),$(MAKECMDGOALS)),)
-include $(wildcard $(BENCHMARK_DIR)/*.d)
endif

# Companion C compiler derived from $(CXX) so C and C++ objects use
# matching toolchains (g++ -> gcc, clang++ -> clang, c++ -> cc). Make
# has a built-in default of "cc" with origin "default", which we
# override here. Explicit CC= on the command line or in the environment
# wins.
ifeq ($(origin CC),default)
CXX_BASENAME := $(notdir $(CXX))
# $(dir foo) returns "./" when $(CXX) has no slash. Strip that so the
# companion compiler name is searched on $PATH.
...
# Combination used for the storage benchmark with no caches.
...
# shared output file. Recipes run scenarios in this fixed order so the
...
# scenario.
#
# Aggregate result files use these names:
...
#
# Cache-independent algorithms ignore the cache flags. Their aggregate
# file uses $(BASELINE_COMBINATION) (c0r0e0l0). The binary is compiled
# with $(REFERENCE_COMBINATION), which has no effect on the
# measurements.
...
# cache combinations. Each combination recipe runs all scenarios in
...
# Restrict workloads: dedicated binaries so full stencil sweeps do not
# bloat the construction/point-query benchmark binaries.
$(eval $(call SCENARIO_BUILD_RULE,restrict-structured-cache-dependent,$(REFERENCE_COMBINATION)))
$(eval $(call SCENARIO_BUILD_RULE,restrict-structured-cache-independent,$(REFERENCE_COMBINATION)))
$(eval $(call SCENARIO_BUILD_RULE,restrict-5d-cache-dependent,$(REFERENCE_COMBINATION)))
$(eval $(call SCENARIO_BUILD_RULE,restrict-5d-cache-independent,$(REFERENCE_COMBINATION)))
.PHONY: benchmark-restrict benchmark-restrict-structured benchmark-restrict-5d
benchmark-restrict-structured: \
	$(BENCHMARK_DIR)/restrict-structured-cache-dependent-$(REFERENCE_COMBINATION) \
	$(BENCHMARK_DIR)/restrict-structured-cache-independent-$(REFERENCE_COMBINATION) | $(RESULT_DIR)
	rm -f $(RESULT_DIR)/restrict-structured-$(REFERENCE_COMBINATION).txt
	@echo "running structured restrict workloads (cache-dependent)"; \
	  $(BENCHMARK_DIR)/restrict-structured-cache-dependent-$(REFERENCE_COMBINATION) > \
	  $(RESULT_DIR)/restrict-structured-$(REFERENCE_COMBINATION).txt
	rm -f $(RESULT_DIR)/restrict-structured-cache-independent-$(BASELINE_COMBINATION).txt
	@echo "running structured restrict workloads (cache-independent)"; \
	  $(BENCHMARK_DIR)/restrict-structured-cache-independent-$(REFERENCE_COMBINATION) > \
	  $(RESULT_DIR)/restrict-structured-cache-independent-$(BASELINE_COMBINATION).txt
benchmark-restrict-5d: \
	$(BENCHMARK_DIR)/restrict-5d-cache-dependent-$(REFERENCE_COMBINATION) \
	$(BENCHMARK_DIR)/restrict-5d-cache-independent-$(REFERENCE_COMBINATION) | $(RESULT_DIR)
	rm -f $(RESULT_DIR)/restrict-5d-$(REFERENCE_COMBINATION).txt
	@echo "running 5D restrict workloads (cache-dependent)"; \
	  $(BENCHMARK_DIR)/restrict-5d-cache-dependent-$(REFERENCE_COMBINATION) > \
	  $(RESULT_DIR)/restrict-5d-$(REFERENCE_COMBINATION).txt
	@echo "running 5D restrict workloads (cache-independent)"; \
	  $(BENCHMARK_DIR)/restrict-5d-cache-independent-$(REFERENCE_COMBINATION) >> \
	  $(RESULT_DIR)/restrict-5d-$(REFERENCE_COMBINATION).txt
benchmark-restrict: benchmark-restrict-structured benchmark-restrict-5d
...
# extractors. Multiple extractors build and run in parallel
# under `make -j`.
$(EXTRACT_BIN_DIR):
	mkdir -p $@
...
# Restriction-ratio summary: extractor writes the LaTeX table directly.
generated/restrict-summary.tex: $(EXTRACT_BIN_DIR)/restrict-summary $(RESULT_FILES) | generated
	$< > $@

PLOTS += generated/restrict-summary.tex

# Multiple-survey pos/at/construct tables: extractors write LaTeX directly.
generated/pos-multiple-survey-table.tex: $(EXTRACT_BIN_DIR)/pos-multiple-survey-table $(RESULT_FILES) | generated
	$< > $@

PLOTS += generated/pos-multiple-survey-table.tex

generated/at-multiple-survey-table.tex: $(EXTRACT_BIN_DIR)/at-multiple-survey-table $(RESULT_FILES) | generated
	$< > $@

PLOTS += generated/at-multiple-survey-table.tex

generated/construct-multiple-survey-table.tex: $(EXTRACT_BIN_DIR)/construct-multiple-survey-table $(RESULT_FILES) | generated
	$< > $@

PLOTS += generated/construct-multiple-survey-table.tex

generated/construct-vs-density-table.tex: $(EXTRACT_BIN_DIR)/construct-vs-density-table $(RESULT_FILES) | generated
	$< > $@

PLOTS += generated/construct-vs-density-table.tex

generated/kappa-vs-density-table.tex: $(EXTRACT_BIN_DIR)/kappa-vs-density-table $(RESULT_FILES) | generated
	$< > $@

PLOTS += generated/kappa-vs-density-table.tex

generated/kappa-multiple-survey-table.tex: $(EXTRACT_BIN_DIR)/kappa-multiple-survey-table $(RESULT_FILES) | generated
	$< > $@

PLOTS += generated/kappa-multiple-survey-table.tex

generated/pos-ratio-vs-density-table.tex: $(EXTRACT_BIN_DIR)/pos-ratio-vs-density-table $(RESULT_FILES) | generated
	$< > $@

PLOTS += generated/pos-ratio-vs-density-table.tex

$(DP_QUALITY_BIN) $(DP_QUALITY_BIN).d &: $(DP_QUALITY_SRC) \
	| $(EXTRACT_BIN_DIR)
	$(CXX) -std=c++23 -O2 -Wall -Wextra -I$(SRC_DIR) \
	    -MMD -MP -MT $(DP_QUALITY_BIN) -MF $(DP_QUALITY_BIN).d \
	    -o $(DP_QUALITY_BIN) $(DP_QUALITY_SRC)

DP_QUALITY_DEPENDENCY_GOALS := \
	paper plots IPT.pdf IPT.bbl \
	generated/dynamic-programming-quality.tex \
	regenerate-dynamic-programming-quality $(DP_QUALITY_BIN)
ifneq ($(filter $(DP_QUALITY_DEPENDENCY_GOALS),$(MAKECMDGOALS)),)
-include $(DP_QUALITY_BIN).d
endif

generated/dynamic-programming-quality.tex: $(DP_QUALITY_BIN) | generated
	$< > $@

PLOTS += generated/dynamic-programming-quality.tex

.PHONY: regenerate-dynamic-programming-quality
regenerate-dynamic-programming-quality: $(DP_QUALITY_BIN) | generated
	$(DP_QUALITY_BIN) > generated/dynamic-programming-quality.tex
